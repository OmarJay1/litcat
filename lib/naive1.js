...
// copied mostly from https://github.com/jbrooksuk/node-summary
...
const splitContentToSentences = (content, callback) => {
...
const splitContentToParagraphs = (content, callback) => {
  callback(content.split('\n\n'))
}

const sentencesIntersection = (sent1, sent2, callback) => {
  let s1 = sent1.split(' ')
  let s2 = sent2.split(' ')
...
const formatSentence = (sentence, callback) => {
...
const getBestSentence = (paragraph, sentences_dict, callback) => {
...
const getSortedSentences = (paragraph, sentences_dict, n, callback) => {
  splitContentToSentences(paragraph, sentences => {
...
const getSentencesRanks = (content, callback, sentences_dict) => {
...
exports.summarize(title, content, function(err, summary) {
...
exports.summarizeFromUrl(url, function(err, summary) {
  if(err) {
